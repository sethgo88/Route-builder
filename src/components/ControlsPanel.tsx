...
interface Props {
  mapViewRef: React.RefObject<MapLibreGL.MapView>;
}
...
export default function ControlsPanel({ mapViewRef }: Props) {
  ...
  const handleExport = useCallback(async () => {
    if (!route || waypoints.length === 0) {
      Alert.alert('Nothing to export', 'Add waypoints and compute a route first.');
      return;
    }
    setIsExporting(true);
    try {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (parsed.waypoints.length === 0) {
        Alert.alert('No waypoints found', 'The GPX file contained no usable waypoints or track points.');
        return;
      }
      ...
      await MapLibreGL.offlineManager.createPack(
        ...
        (_region, status) => {
          ...
          if (status.percentage >= 100) {
            ...
            Alert.alert('Download complete', 'This region is now available offline.');
          }
        },
        (_region, error) => {
          setOfflineProgress(null);
    ...
    >
      <BottomSheetScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.controls}>
          ...
          {/* Offline download */}
          <TouchableOpacity
            style={[styles.downloadButton, offlineProgress !== null && styles.downloadActive]}
...
function ActionButton({ label, onPress, disabled, destructive }: ActionButtonProps) {
  return (
    <TouchableOpacity
      style={[
        styles.actionButton,
